...
3.\ At the moment the stimuli stays on the screen until a response. To increase errors and speed up responses can you set a time limit so that the participant only has 2 seconds to respond? 
`r hide("Hint")`
```{r,echo = FALSE, results='asis'}
...
`r hide("Hint 2")`
```{r,echo = FALSE, results='asis'}
cat("In the excel spreadsheet add a new column called 'correct' include in there the correct key response for each trial. In the Data tab of the 'key_resp' component, click 'Store correct' and enter the name of your column in the 'Correct answer' field")
```
`r unhide()`
